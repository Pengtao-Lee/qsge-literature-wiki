...
`raw_markdown/` holds machine-readable conversions of the source PDFs. The default conversion tool is [markitdown](../Users/xueheng/.claude/skills/markitdown/SKILL.md), which produces structured Markdown from PDFs. Conversion is done without academic interpretation — the parse layer is faithful to the source, not editorialized.
