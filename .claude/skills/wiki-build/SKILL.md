...
```bash
mkdir -p /tmp/wiki-build-overlay
rsync -a wiki/ /tmp/wiki-build-overlay/
rsync -a agent_tasks/<workspace-slug>/round-N/round-output/ /tmp/wiki-build-overlay/
python scripts/check_links.py /tmp/wiki-build-overlay raw_markdown > round-N/lint-report.md
python scripts/validate_frontmatter.py /tmp/wiki-build-overlay >> round-N/lint-report.md
```
